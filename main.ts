...
controller.B.onEvent(ControllerButtonEvent.Pressed, function () {
    game.showLongText("" + "Tu inventario es:\n" + PlayerInventoryLlenya + " llenys", DialogLayout.Bottom)
    if (PlayerInventory.length > 0) {
        for (let x = 0; x <= PlayerInventory.length / 2; x++) {
            game.showLongText("" + PlayerInventory[x * 2] + " " + PlayerInventory[x * (2 + 1)], DialogLayout.Bottom)
        }
    }
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.NPC, function (sprite2, otherSprite) {
    ...
    if (controller.A.isPressed()) {
        ...
        if (game.ask("Vols " + NPC_inventari[2 * multiplicador_array] + ("? Tinc " + NPC_inventari[2 * multiplicador_array + 1]))) {
            ...
            if (input2 <= parseFloat(NPC_inventari[2 * multiplicador_array + 1])) {
                if (PlayerInventoryLlenya >= input2) {
                    game.splash("Genial! Aquí tens!")
                    PlayerInventoryLlenya += PlayerInventoryLlenya - input2
                    PlayerInventory.push(NPC_inventari[2 * multiplicador_array])
                    PlayerInventory.push(NPC_inventari[2 * multiplicador_array + 1])
                } else {
